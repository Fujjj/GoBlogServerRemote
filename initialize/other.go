...
func InitOther() {
	// 解析刷新令牌过期时间
	refreshTokenExpiry, err := utils.ParseDuration(global.Configs.Jwt.RefreshTokenExpiryTime)
	if err != nil {
	...
	// 解析访问令牌过期时间
	_, err = utils.ParseDuration(global.Configs.Jwt.AccessTokenExpiryTime)
	if err != nil {
