...
func (website *WebsiteApi) WebsiteLogo(c *gin.Context) {
	if global.Configs.Website.Logo != "" {
		c.Redirect(http.StatusMovedPermanently, global.Configs.Website.Logo)
	} else {
...
func (website *WebsiteApi) WebsiteTitle(c *gin.Context) {
	c.JSON(http.StatusOK, gin.H{"title": global.Configs.Website.Title})
}

// WebsiteInfo 获取网站配置信息
func (website *WebsiteApi) WebsiteInfo(c *gin.Context) {
	response.OkWithData(global.Configs.Website, c)
}
...
func (website *WebsiteApi) WebsiteCalendar(c *gin.Context) {
	...
	calendar, err := websiteService.WebsiteCalendar(c.Request.Context(), dateStr)
	if err != nil {
